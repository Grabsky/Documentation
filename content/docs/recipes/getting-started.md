---
title: Getting Started
type: docs
weight: 1
---

**Recipes** is a plugin that allows to create new, or override existing recipes. Wide range of recipe types are supported.

{{< cards cols="3">}}
  {{< card link="https://modrinth.com/plugin/recipes-plugin" title="Modrinth" icon="modrinth" >}}
  {{< card link="https://github.com/Grabsky/Recipes" title="GitHub" icon="github" >}}
  {{< card link="https://discord.com/invite/PuzqF2Yd5q" title="Discord" icon="discord" >}}
{{< /cards >}}


<br>

## Features

- **File Based**  
  Define recipes using JSON files.

- **Components Support**  
  Use **#tags** and 1.20.6+ **components** in item definitions.

- **Custom Items**  
  Easily add and update custom items using built-in custom item registry.

- **Override Existing Recipes**  
  Override other recipes by customizing recipes' namespace.

<br/>

## Requirements
Plugin runs only on **Paper** (or **Folia**) **1.21.1** and above, powered by **Java 21** or higher.

<br>
